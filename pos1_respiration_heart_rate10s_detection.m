...
% of 0.1 Hz for 12 cycles of inhalation and exhalation 
...
% file: signals/pos1_accelerometer_data_10sbreathing.txt
%
% They were recorded with a sampling rate of 202 Hz and 
% a resolution of 0.244 mg/LSB

fprintf('Loading of pos1_accelerometer_data_10sbreathing.txt  \n');
load signals/pos1_accelerometer_data_10sbreathing.txt % data composed by the following columns: time, gFx, gFy, gFz, TgF

fprintf('Loading of pos2_accelerometer_data_10sbreathing.txt  \n');
load signals/pos2_accelerometer_data_10sbreathing.txt % data composed by the following columns: time, gFx, gFy, gFz, TgF

%% Data acquisition step
fprintf('*Data acquisition step*  \n');

% remove time values duplicated
[~,uidx] = unique(pos1_accelerometer_data_10sbreathing(:,1),'stable');
pos1_accelerometer_data = pos1_accelerometer_data_10sbreathing(uidx,:);

[~,uidx] = unique(pos2_accelerometer_data_10sbreathing(:,1),'stable');
pos2_accelerometer_data = pos2_accelerometer_data_10sbreathing(uidx,:);
...
figure(1)
signals_plotting(pos1_accelerometer_data, pos1_timeAxis, 'Pos1-accelerometer-data-10sbreathing')

figure(2)
signals_plotting(pos2_accelerometer_data, pos2_timeAxis, 'Pos2-accelerometer-data-10sbreathing')
...
figure(4)
signals_filtered_plotting(pos1_accelerometer_data, pos1_accelerometer_data_filtered, pos1_timeAxis, 'Pos1-accelerometer-data-10sbreathing')
...
figure(5)
yAxis_filtered_plotting(pos1_y, pos1_y_filtered, pos1_timeAxis, 'Pos1-accelerometer-data-10sbreathing')


%% Time domain analysis: Detection of minima of y axis of the filtered acceleration data

half_breath_duration = 5; % 5 seconds represent half of the entire breath duration
...
title('Pos1-accelerometer-data-10sbreathing filtered - minima detection')
xlabel('time (s)')
ylabel('Y (mg/LSB)')
...
figure(9)
semilogy(f,pxx,'b', f(maximum_peak_index), maximum_peak, 'r.', 'MarkerSize',20)
...
% plot the original data and the filtered data
figure(11)
signals_filtered_plotting(pos1_accelerometer_data, pos1_accelerometer_data_bandpassed, pos1_timeAxis, 'Pos1-accelerometer-data-10sbreathing')

% plot the y axis of the original and the filtered data
pos1_z = pos1_accelerometer_data(:,4);
pos1_z_filtered = pos1_accelerometer_data_bandpassed(:,4);
figure(12)
zAxis_filtered_plotting(pos1_z, pos1_z_filtered, pos1_timeAxis, 'Pos1-accelerometer-data-10sbreathing')
...
% plot the original data and the filtered data
figure(14)
signals_filtered_plotting(pos2_accelerometer_data, pos2_accelerometer_data_bandpassed, pos2_timeAxis, 'Pos2-accelerometer-data-10sbreathing')

% plot the y axis of the original and the filtered data
pos2_y = pos2_accelerometer_data(:,3);
pos2_y_filtered = pos2_accelerometer_data_bandpassed(:,3);
figure(15)
yAxis_filtered_plotting(pos2_y, pos2_y_filtered, pos2_timeAxis, 'Pos2-accelerometer-data-10sbreathing')

%% Plotting the signals filtered to detect the heart rate

figure(16)
signals_heart_rate_plotting_1(pos1_accelerometer_data, pos1_accelerometer_data_bandpassed, pos2_accelerometer_data, pos2_accelerometer_data_bandpassed, pos1_timeAxis, pos2_timeAxis, 'Pos1-accelerometer-data-10sbreathing', 'Pos2-accelerometer-data-10sbreathing')

figure(17)
signals_heart_rate_plotting_2(pos1_z_filtered, pos2_y_filtered, pos1_timeAxis, pos2_timeAxis, 'accelerometer-data-10sbreathing')
